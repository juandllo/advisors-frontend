...
export function DataTableViewOptions<TData>({
  ...
  const handleIsRealoading = (isReloading: boolean) => {
    ...
    alertDialog("Actualización de Pedidos", "Estas seguro?")
      .then(result => {
        if (result.isConfirmed) {
          alertSuccessTimer("Se han actualizado los pedidos.");
        }
      });
  }

  const handleSinchronizingData = (isSinchronizing: boolean) => {
    //updateSinchronizingPlayers(isSinchronizing)
    alertSuccessTimer("Se han sincronizado los pedidos.");
  } 
